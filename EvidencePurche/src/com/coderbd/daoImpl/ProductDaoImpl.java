...
public class ProductDaoImpl implements ProductDao {
    ...
    Connection conn = Connect.GetDataConnect();
    ...
    public void createTable() {
        String sql = "create table if not exists productBuy (id int(11) auto_increment primary key, product_name varchar(50),product_code varchar(50),quantity int(11), unit_price double, total_price double)";
        try {
    ...
    public void save(Product p) {
        String sql = "insert into productBuy(product_name,product_code,quantity,unit_price,total_price) values (?,?,?,?,?)";
        try {
            PreparedStatement pstm = conn.prepareStatement(sql);
    ...
    public Product getProductByCode(String code) {
        ...
        String sql = "select * from productBuy where product_code code = ?";
        try {
            PreparedStatement pstm = conn.prepareStatement(sql);
    ...
    public List<Product> getList() {
        ...
        String sql = "select * from productBuy";
        try {
            PreparedStatement pstm = conn.prepareStatement(sql);
            ResultSet rs = pstm.executeQuery();
            while (rs.next()) {
