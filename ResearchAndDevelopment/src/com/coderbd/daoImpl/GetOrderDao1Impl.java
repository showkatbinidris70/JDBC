...
public class GetOrderDao1Impl implements GetOrderDao1 {
    ...
    public void save(GetOrder1 g) {
        ...
        try {
            ...
            pstm.setString(6, g.getDeliveryDate());
            pstm.executeUpdate();
    ...
    public GetOrder1 getGetOrder1ByGetOrder1Code(String code) {
        ...
        try {
            ...
            while (rs.next()) {
                getOrder1 = new GetOrder1(rs.getString(1), rs.getInt(2), rs.getDouble(3), rs.getDouble(4), rs.getDate(5), rs.getString(6));
            }
        } catch (SQLException ex) {
            Logger.getLogger(GetOrderDao1Impl.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public List<GetOrder1> getList() {
        ...
        try {
            ...
            while (rs.next()) {
                GetOrder1 go = new GetOrder1(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getDouble(4), rs.getDouble(5), rs.getDate(6), rs.getString(7));
                list.add(go);
            }
        } catch (SQLException ex) {
            Logger.getLogger(GetOrderDao1Impl.class.getName()).log(Level.SEVERE, null, ex);
